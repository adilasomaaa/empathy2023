...
const routes: RouteRecordRaw[] = [
    ...
    },
    {
        ...
        children: [
            ...
            {
                path: '/master-rumahsakit',
                name: 'master-rumahsakit',
                component: () => import(/* webpackChunkName: "analytics" */ '../views/pages/rumahsakit/index.vue'),
                meta: {
                    role: 'all',
                    ability: null
                }
...
router.beforeEach((to, from, next) => {
    ...
    if (to.name === 'login' && auth.authenticated){
        ...
        if (!auth.authenticated) {
            ...
            // Ini adalah kondisi untuk mengecek apakah halaman yang di akses sesuai dengan role yang dimiliki
            if(auth.role.includes(to.meta.role) || to.meta.role == 'all'){
                // let ability = JSON.parse(JSON.stringify(auth.ability));
                // if(to.meta.ability === null || ability.some(r => to.meta.ability.includes(r))){
                //     next();
                // }else{
                //     next({
                //         name: 'notfound'
